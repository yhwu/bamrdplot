...
void get_pairend_info(samfile_t *fp_in, bam_index_t *bamidx, int ref, 
		      int& insert, int& insert_sd)
{
  insert=insert_sd=-1;
  ...
  if ( ref<0 ) ref=0;
  iter = bam_iter_query(bamidx, ref, 1000, 0x7fffffff);
  ...
  if (count>0) l_qseq=l_qseq_sum/count+0.5;
    ...
    cerr << "cannot find any reads on chromosome: "
	 << fp_in->header->target_name[ ref ] << endl; 
    return;
  }
  ...
  if ( isize_c>2 ) {
    ...
    insert=(int)isize;
    insert_sd=(int)isize_sd;
  }
  
  cerr << "sampled from " << isize_c << " reads" << endl
       << "bam_target\t" << fp_in->header->target_name[ ref ] << "\n"
       << "bam_target_tid\t" << ref << "\n"
       << "bam_pe_insert\t" << insert << "\n"
       << "bam_pe_insert_sd\t" << insert_sd << "\n"
       << endl;
